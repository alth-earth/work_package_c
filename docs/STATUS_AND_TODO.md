...
| 工程复验 | `UV_OFFLINE=1 make check`：`138 passed, 1 skipped`，Ruff/lock/sync/CLI 均通过 |
| 整体状态 | 进行中：工程主线完成，实源/科学/D 验收未完成 |
| 安全口径 | 科研演示；calibration 为 `demo_unvalidated` |

唯一 skipped 是 `tests/integration/test_legacy_adapter.py` 的可选外部旧制品测试；它要求
本机存在历史 `/mnt/c/.../交付包.zip`。当前正式 v2/v3 测试没有因此跳过。
...
3. 评估是否退役硬编码旧 ZIP 的可选回归；决定前保持 skipped 透明可见。
