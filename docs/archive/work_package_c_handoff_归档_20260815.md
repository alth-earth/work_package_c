> [!NOTE]
> **二次文档治理归档声明**
> - 本文件角色：2026-08-15 改造前的 C handoff 快照，仅供历史追溯。
> - 归档时间：2026-08-15（Asia/Shanghai）。
> - 现行文件：[work_package_c_handoff.md](work_package_c_handoff.md)。
> - 归档原因：落实挑战杯演示目标、两种运行模式、散货船演示参数和非阻塞科学接口。
...
当前本地复验命令 `UV_OFFLINE=1 make check` 的结果为 `138 passed`。已按用户确认退役 C 中
硬编码 `交付包.zip` 的外部制品回归；legacy adapter 仍保留显式开发模式门禁，但不再依赖
已无用途的旧 B 交付包。
