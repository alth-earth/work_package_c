"""Internal resumable sessions for the experimental temporal-label planner.

This module is intentionally not imported from ``planners.__init__``.  It is a
small P1 execution boundary: every mutable search object belongs to exactly one
objective/session, checkpoints are in-process immutable values, and restoring a
checkpoint requires the complete input/configuration identity to match.
"""
